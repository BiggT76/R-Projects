...
vw <- tbl(conn,"vwReportKeyData") %>% 
  select(Brand, OrderStatus, ScheduledCompleteDate, Plant, ReceivedDate, OrderCaseTotal, ShopFloorNumber, OrderTotal, ProductLine) %>% 
...
t <- vw %>%
  filter(!is.na(ReceivedDate) & !is.na(OrderCaseTotal) & OrderCaseTotal != 0 & (!str_detect(ShopFloorNumber, "TEST")))  %>% 
...
```{r}
sked <- inner_join(t,LineMap,by= c('JobPrefix','ProductLine')) %>%
...
```{r, echo=FALSE}
projected <- inner_join(t,LineMap,by = c("JobPrefix","ProductLine")) %>% 
  filter(weekNumeric == 99) %>%
